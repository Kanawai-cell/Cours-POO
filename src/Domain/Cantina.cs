...
namespace Domain
{
    ...
    public sealed class Cantina
    {
        ...
        public void AddReservation(Reservation reservation)
        {
            ...
            {
                throw new CantinaException(message: "Reservation not in schedule");
            }

            if (_reservedUsers.Contains(reservation.User))
            {
                throw new CantinaException(message: "User already reserved");
            }

            //if (_reservedUsers.Count >= MaxUsers)
            //{
            //    throw new CantinaException(message: "Too much people !");
            //}

            var collideCount = 0;
            foreach( var r in _reservations.Values)
            {
                if(r.TimeSlot.Collide(reservation.TimeSlot))
                {
                    collideCount++;
                    if(collideCount == MaxUsers)
                    {
                        throw new CantinaException(message: "Too much people !");
                    }
                }
